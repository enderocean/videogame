...
func _ready() -> void:
	...
	if OS.get_name() == "Windows":
		if not file.file_exists("user://ardusub.exe"):
			...
			download_file(
				"https://firmware.ardupilot.org/Tools/MissionPlanner/sitl/ArduSub.elf",
				"user://ardusub.exe"
			)
			download_file(
				"https://firmware.ardupilot.org/Tools/MissionPlanner/sitl/cyggcc_s-1.dll",
				"user://cyggcc_s-1.dll"
			)
			download_file(
				"https://firmware.ardupilot.org/Tools/MissionPlanner/sitl/cygstdc++-6.dll",
				"user://cygstdc++-6.dll"
			)
			download_file(
				"https://firmware.ardupilot.org/Tools/MissionPlanner/sitl/cygwin1.dll",
				"user://cygwin1.dll"
			)
		...
		if not file.file_exists("user://ardusub"):
			print("SITL binary not found, downloading...")
			download_file(
				"https://firmware.ardupilot.org/Sub/latest/SITL_x86_64_linux_gnu/ardusub",
				"user://ardusub"
			)
	else:
