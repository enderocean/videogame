...
func request_send_score(username: String, level_id: String, time: int, score: int) -> void:
	...
	if token_leaderboard.empty():
		...
		if success:
			token_leaderboard = token
			emit_signal("token_leaderboard_success")
			print("Token received: ", token_leaderboard)
		else:
	...
	add_child(requester)
	requester.request()
	print("Sending score: ", token_leaderboard)
	...
	if success:
		...
		emit_signal("score_failed", error)
...
func _ready() -> void:
	...
	if not file.file_exists(config_path):
		...
		leaderboard_url = config.get_value("LEADERBOARD", "url", "")
		leaderboard_key = config.get_value("LEADERBOARD", "key", "")
		leaderboard_secret = config.get_value("LEADERBOARD", "secret", "")
