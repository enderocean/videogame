...
func create_attach(pin):
	...
	var new_pos = soft_rope_2.global_transform.origin.linear_interpolate(soft_rope_1.global_transform.origin, 0.5)
	soft_rope_2.global_transform.origin = new_pos

	soft_rope_2.set_rope_pins()
	mdt.create_from_surface(soft_rope_2.mesh, 0)
	
	var diff = soft_rope_2.global_transform.xform(mdt.get_vertex(soft_rope_2.pins_beg[0])).distance_squared_to(soft_rope_1.global_transform.origin)
	var length = soft_rope_2.global_transform.xform(mdt.get_vertex(soft_rope_2.pins_end[0])).distance_squared_to(soft_rope_1.global_transform.origin) - diff
